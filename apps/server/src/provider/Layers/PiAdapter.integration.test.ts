...
interface FakePiTransport {
  ...
  readonly pushResponse: (response: RpcResponse) => Effect.Effect<void>;
...
const makeFakePiRpcTransport = Effect.gen(function* () {
  ...
  const transport: PiRpcTransport = {
    ...
    request: (command, id) =>
      Effect.gen(function* () {
        commands.push(command);
        const response = responses.get(command.type);
        if (response) yield* Queue.offer(messages, { _tag: "response", id, response });
        return response;
      }),
    messages,
    kill: Effect.void,
  ...
  return {
    ...
    pushResponse: (response) =>
      Queue.offer(messages, { _tag: "response", id: undefined, response }).pipe(Effect.asVoid),
...
it.layer(HarnessLayer)("PiAdapter integration", (it) => {
  ...
  it.effect("does not finalize on agent_end; waits for agent_settled", () =>
    Effect.gen(function* () {
      const { adapter, fake } = yield* makePiAdapterForTest(enabledSettings());
  ...
  it.effect("uses Pi's configured model and expands discovered skills", () =>
    Effect.gen(function* () {
      const { adapter, fake } = yield* makePiAdapterForTest(enabledSettings());
      fake.setResponse(
        "get_commands",
        asResponse({
          ...
          command: "get_commands",
          success: true,
          data: { commands: [{ name: "skill:review", source: "skill" }] },
        }),
      );
      const threadId = ThreadId.make("pi-native-settings");
      yield* adapter.startSession({
        threadId,
        runtimeMode: "full-access",
        modelSelection: {
      ...
      yield* adapter.sendTurn({ threadId, input: "Please $review this change", attachments: [] });
      expect(fake.commands).toContainEqual({
        type: "prompt",
        message: "/skill:review Please  this change",
      });
  ...
  it.effect("surfaces prompt rejection instead of leaving a turn running", () =>
    Effect.gen(function* () {
      const { adapter, fake } = yield* makePiAdapterForTest(enabledSettings());
      const threadId = ThreadId.make("pi-prompt-rejected");
      const collected = yield* collectEvents(
        adapter,
        threadId,
        (event) => event.type === "turn.completed",
  ...
  it.effect("routes manual compaction through RPC and completes after its response", () =>
    Effect.gen(function* () {
      const { adapter, fake } = yield* makePiAdapterForTest(enabledSettings());
      const threadId = ThreadId.make("pi-compact");
      const collected = yield* collectEvents(
        adapter,
        threadId,
        (event) => event.type === "turn.completed",
      ...
      yield* adapter.startSession({ threadId, runtimeMode: "full-access" });
      yield* adapter.sendTurn({ threadId, input: "/compact keep the decisions", attachments: [] });
      expect(fake.commands).toContainEqual({
        type: "compact",
        customInstructions: "keep the decisions",
      });
      yield* fake.pushResponse(
        asResponse({ type: "response", command: "compact", success: true, data: {} }),
      );
      yield* Fiber.join(collected.fiber);
      const events = yield* Ref.get(collected.store);
      expect(events.find((event) => event.type === "turn.completed")?.payload).toMatchObject({
  ...
  it.effect("updates the native resume cursor after rollback", () =>
    Effect.gen(function* () {
      const { adapter, fake } = yield* makePiAdapterForTest(enabledSettings());
      const threadId = ThreadId.make("pi-rollback-native-session");
      yield* adapter.startSession({ threadId, runtimeMode: "full-access" });
      fake.setResponse(
        "get_fork_messages",
        asResponse({
          ...
          data: {
            messages: [
              { entryId: "one", text: "first" },
              { entryId: "two", text: "second" },
            ],
          },
        }),
      );
      ...
      fake.setResponse(
        ...
        asResponse({
          ...
          command: "get_state",
          success: true,
          data: { sessionFile: "/tmp/pi-rolled-back.jsonl" },
        }),
      );
      yield* adapter.rollbackThread(threadId, 1);
      const sessions = yield* adapter.listSessions();
      expect(sessions[0]?.resumeCursor).toEqual({ sessionFile: "/tmp/pi-rolled-back.jsonl" });
      yield* adapter.stopSession(threadId);
    }),
  );
  it.effect("settles an extension-only slash command without an agent run", () =>
    Effect.gen(function* () {
      const { adapter, fake } = yield* makePiAdapterForTest(enabledSettings());
      const threadId = ThreadId.make("pi-extension-command");
      const collected = yield* collectEvents(
        adapter,
        threadId,
        (event) => event.type === "turn.completed",
      ...
      yield* adapter.startSession({ threadId, runtimeMode: "full-access" });
      yield* adapter.sendTurn({ threadId, input: "/extension-command", attachments: [] });
      yield* fake.pushResponse(asResponse({ type: "response", command: "prompt", success: true }));
      yield* Fiber.join(collected.fiber);
      const events = yield* Ref.get(collected.store);
      expect(events.find((event) => event.type === "turn.completed")?.payload).toMatchObject({
        state: "completed",
