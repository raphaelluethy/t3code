...
import { piRecordField, piRecordNumber } from "../pi/PiRpc.ts";
...
export const makePiAdapter = Effect.fn("makePiAdapter")(function* (
  ...
  const reportUsage = (context: PiSessionContext, usage: unknown, contextUsage?: unknown) =>
    Effect.gen(function* () {
      const used = piRecordNumber(contextUsage, "tokens") ?? piRecordNumber(usage, "totalTokens");
      const maximum = piRecordNumber(contextUsage, "contextWindow") ?? context.contextWindow;
      if (
        used === undefined ||
        used < 0 ||
        maximum === undefined ||
        maximum <= 0 ||
        used === context.lastUsedTokens
      )
        return;
      context.lastUsedTokens = used;
      yield* offerRuntimeEvent({
        ...
        payload: { usage: { usedTokens: Math.trunc(used), maxTokens: Math.trunc(maximum) } },
      });
    });
  ...
  const handlePiEvent = (
    ...
    Effect.gen(function* () {
      ...
      switch (event.type) {
        case "agent_start": {
          if (context.turnState) context.turnState.sawAgentActivity = true;
          yield* offerRuntimeEvent({
            ...base,
        ...
        case "message_update": {
          ...
          yield* reportUsage(
            context,
            piRecordField(event, "usage") ?? piRecordField(event.message, "usage"),
          );
        ...
        case "turn_end": {
          // Pi emits several low-level turns before agent_settled.
          return;
        }
          ...
          if (event.message.role === "assistant") {
            context.failureMessage =
              event.message.stopReason === "error"
                ? (event.message.errorMessage ?? "Pi could not complete the response.")
                : undefined;
          }
          return;
        }
        ...
        case "agent_settled": {
          ...
          if (
            ...
            return;
          const stats = yield* context.transport.request(
            { type: "get_session_stats" },
            `pi-stats-${yield* nextUuid}`,
            2_000,
          );
          if (stats?.success && stats.command === "get_session_stats") {
            yield* reportUsage(context, stats.data.tokens, stats.data.contextUsage);
          }
          yield* completeTurn(
            context,
            context.failureMessage ? "failed" : "completed",
            context.failureMessage,
          );
          return;
        }
  ...
  const handleMessage = (
    ...
    switch (message._tag) {
      ...
      case "response": {
        ...
        if (!response.success && ["prompt", "compact"].includes(response.command)) {
          return completeTurn(context, "failed", response.error);
        }
        ...
        if (
          ...
          // Queue the probe reply behind native events so a command that starts
          // agent work cannot be completed before its output is consumed.
          return context.transport
            .request({ type: "get_state" }, `pi-command-idle-${turn.turnId}`, PI_STATE_TIMEOUT_MS)
        ...
        if (
          ...
          response.command === "get_state" &&
          turn &&
          message.id === `pi-command-idle-${turn.turnId}` &&
          !response.data.isStreaming &&
          !response.data.isCompacting &&
          !(response.data.pendingMessageCount > 0)
        ) {
        ...
        return response.success && response.command === "compact"
  ...
  const startSession: PiAdapterShape["startSession"] = Effect.fn("startSession")(function* (input) {
    ...
    const resolvedArgs = resolvePiLaunchArgs(piSettings.launchArgs);
    if (!resolvedArgs.ok)
      return yield* new ProviderAdapterValidationError({
        provider: PROVIDER,
        operation: "startSession",
        issue: resolvedArgs.message,
      });
    const extensionPath = yield* materializePiT3McpExtension(
      `${serverConfig.baseDir}/caches/pi`,
    ).pipe(
      ...
      Effect.mapError(
        (cause) =>
          new ProviderAdapterProcessError({
            provider: PROVIDER,
            threadId,
            detail: "Failed to prepare Pi's T3 extension.",
            cause,
          }),
      ),
    );
    ...
    const processEnv = launch.env;
    const verifyApprovalGate = input.runtimeMode !== "full-access";
    ...
    if (stateResponse?.success && stateResponse.command === "get_state") {
      ...
      context.defaultModel = model ? `${model.provider}/${model.id}` : undefined;
      context.currentModel = context.defaultModel;
      context.appliedThinkingLevel = stateResponse.data.thinkingLevel;
    }
    yield* maybeSwitchPiModel(
      context,
      modelSelection?.model === "default" ? context.defaultModel : modelSelection?.model,
    );
    ...
    if (commandsResponse?.success && commandsResponse.command === "get_commands") {
      for (const skill of parsePiDiscoveredCommands(commandsResponse.data).skills)
        context.skillNames.add(skill.name);
    }
    if (
      verifyApprovalGate &&
      !piResponseHasCommand(commandsResponse, PI_APPROVAL_SENTINEL_COMMAND)
    ) {
  ...
  const sendTurn: PiAdapterShape["sendTurn"] = Effect.fn("sendTurn")(function* (input) {
    ...
    const requestedModel =
      input.modelSelection?.instanceId === boundInstanceId
        ...
        : undefined;
    const promptText = expandPiSkillReference(
    ...
    if (!context.turnState) {
      context.failureMessage = undefined;
      ...
      context.turnState = {
        ...
        startedAt,
        items: [],
        commandOnly: /^\/(?!skill:)/.test(promptText.trim()),
    ...
    yield* context.transport
      .writeCommand(
        parsePiCompactCommand(promptText) !== null
          ? { type: "compact", ...parsePiCompactCommand(promptText) }
          : buildPiTurnCommand({ isMidTurn, message: promptText, images }),
      )
      .pipe(
        Effect.catchCause((cause) =>
