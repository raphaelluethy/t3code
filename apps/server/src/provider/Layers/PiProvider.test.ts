...
describe("PiProvider", () => {
  it("labels discovered models with the Pi provider they come from", () => {
    const models = parseDiscoveredModels(
      {
        models: [
          { id: "gpt-5.4", name: "GPT-5.4", provider: "openai-codex" },
          { id: "gpt-5.4", name: "GPT-5.4", provider: "cursor" },
          { id: "gpt-5.4", name: "GPT-5.4", provider: "cursor" },
          { id: "orphan", name: "No provider" },
        ],
      },
      undefined,
    );
    assert.deepEqual(
      models.map((model) => [model.slug, model.name, model.subProvider]),
      [
        ["openai-codex/gpt-5.4", "GPT-5.4", "openai-codex"],
        ["cursor/gpt-5.4", "GPT-5.4", "cursor"],
      ],
    );
  });
