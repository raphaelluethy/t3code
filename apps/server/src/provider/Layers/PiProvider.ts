...
import { buildPiRpcLaunch, resolvePiLaunchArgs } from "../pi/piT3McpInjection.ts";
import {
  makePiRpcConnection,
...
/**
 * Pi aggregates many upstream providers (openai-codex, xai, cursor, bridges),
 * and their model names collide ("GPT-5.4" exists under several). The Pi
 * provider id becomes `subProvider` so pickers can show where a model lives.
 */
